...
pub fn wavelet_amplitude(p: Vec2, freq: f32, initial_phase: f32) -> f32 {
    ...
    let amp_env = (1.0 - p.magnitude()).max(0.0);
...
pub fn wavelet_noise(p: Vec2, seed: u32) -> f32 {
    let pfract = p.fract();
    let pfloor = p - pfract;
    ...
    for i in (x-1)..=(x+1) {
        for j in (y-1)..=(y+1) {
            let seed = khash(seed.wrapping_add((i as u32).wrapping_mul(2138242137)).wrapping_add((j as u32).wrapping_mul(712214797)));
            seeds[idx] = seed;
            let mut rng = Rng::new_seeded(seed);
            let c = vec2(rng.next_float(), rng.next_float()) + vec2(i as f32, j as f32);
            let theta = rng.next_angle();
            let freq = rng.uniform_float(2.5, 11.5);
            // remember exponential distribution good
            // all about shifting the distributions
            let initial_phase = rng.next_angle();
            let rt = rng.uniform_float(0.5, 1.0);
            let rn = rng.uniform_float(0.8, 1.2);
...
pub fn fractal_wavelet_noise(mut p: Vec2, octaves: usize, a_scale: f32, f_scale: f32, seed: u32) -> f32 {
    let mut a = 1.0;
    let mut acc = 0.0;
    for i in 0..octaves {
        let seed = khash(seed + i as u32 * 218732137);
        acc += a * wavelet_noise(p, seed);
        a *= a_scale;
        p = p*f_scale;
    }
    acc
...
pub fn test_fractal_wavelet_noise() {
    ...
    for i in 0..w {
        for j in 0..h {
            let p = vec2(i as f32 / w as f32, j as f32 / h as f32);
            let p = 20.0 * p;
            let height = fractal_wavelet_noise(p, 4, 0.5, 2.0, 69);
            let k = height / 2.0 + 0.5;
            buf.set(i, j, vec4(k, k, k, 1.0));
        }
    }
    ...
    for i in 0..w {
        for j in 0..h {
            let p = vec2(i as f32 / w as f32, j as f32 / h as f32);
            let p = 20.0 * p;
            let height = fractal_wavelet_noise(p, 6, 0.6, 1.5, 41);
            let k = height / 2.0 + 0.5;
            buf.set(i, j, vec4(k, k, k, 1.0));
        }
    }
